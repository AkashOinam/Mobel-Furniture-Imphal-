...
export async function POST(request: Request) {
  try {
    ...
    // Update Text Fields
    if (name !== undefined) {
      content = updateStringProperty('name', name, content, startIdx, limitIdx);
    }
    if (description !== undefined) {
      content = updateStringProperty('description', description, content, startIdx, limitIdx);
    }

    // Update Specifications
    const specsKeywordIdx = content.indexOf('specifications:', startIdx);
    if (specsKeywordIdx !== -1 && specsKeywordIdx < limitIdx) {
    ...
    // Update Main Image
    if (imageUrl) {
      content = updateStringProperty('image', imageUrl, content, startIdx, limitIdx);
    }
